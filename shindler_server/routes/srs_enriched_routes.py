"""
SRS Enriched KPI API Routes
FastAPI routes for SRS Enriched (Safety Reporting System) KPIs with date filtering support
"""
...
# Create router
router = APIRouter(prefix="/api/v1", tags=["SRS Enriched KPIs"])


@router.get("/srs-enriched")
async def get_srs_enriched_kpis(
    ...
    """
    Get all SRS Enriched (Safety Reporting System) KPIs with optional date filtering
    
    This endpoint returns comprehensive safety KPIs from the SRS Enriched App data including:
    ...
    **Default Behavior**: If no date parameters are provided, returns data for the last 1 year.
    ...
    try:
        ...
        logger.info(f"Processing SRS Enriched KPI request with date range: {start_date} to {end_date}")
        
        # Initialize KPI queries (SRS Enriched doesn't use date filtering in constructor)
        kpi_queries = SRSEnrichedKPIQueries()
        
        # Execute all KPIs
        results = kpi_queries.get_all_kpis()
        
        logger.info("SRS Enriched KPI request completed successfully")
        
        return {
            "status": "success",
            "message": "SRS Enriched KPIs retrieved successfully",
            "data": results,
            "date_range": {
        ...
        logger.error(f"Error processing SRS Enriched KPI request: {str(e)}")
        raise HTTPException(
            status_code=500,
...
@router.post("/srs-enriched/insights/generate-more")
async def generate_more_srs_enriched_insights(
    request_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Generate additional AI insights for SRS Enriched data that haven't been generated before
    ...
    """
    try:
        logger.info("Processing SRS Enriched generate more insights request")
        ...
        logger.info(f"Generated {len(additional_insights)} additional SRS Enriched insights")
        ...
        logger.error(f"Error generating additional SRS Enriched insights: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error: {str(e)}"
        )
...
@router.get("/srs-enriched/insights")
async def get_srs_enriched_insights(
    start_date: Optional[str] = Query(
        None, 
        description="Start date for filtering (YYYY-MM-DD format). Defaults to 1 year ago if not provided."
    ),
    ...
    """
    Generate AI-powered insights from SRS Enriched safety KPIs
    
    This endpoint analyzes SRS Enriched safety KPI data using Azure OpenAI to generate:
    ...
    try:
        ...
        logger.info(f"Processing SRS Enriched AI insights request with date range: {start_date} to {end_date}")
        ...
        logger.info("SRS Enriched AI insights request completed successfully")
        ...
        logger.error(f"Error processing SRS Enriched AI insights request: {str(e)}")
        raise HTTPException(
            status_code=500,
