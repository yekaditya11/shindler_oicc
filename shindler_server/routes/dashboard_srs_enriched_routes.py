"""
SRS Enriched Dashboard API Routes
FastAPI routes for SRS enriched dashboard KPIs with date filtering and regional support
"""
...
from dashboard.srs_enriched_dashboard_service import SRSEnrichedDashboardService

logger = logging.getLogger(__name__)

# Create router
router = APIRouter(prefix="/dashboard", tags=["SRS Enriched Dashboard"])


@router.get("/srs-enriched")
async def get_srs_enriched_dashboard(
    ...
    """
    Get SRS enriched dashboard data with comprehensive KPIs
    
    This endpoint returns standardized dashboard data from the SRS enriched (Safety Reporting System) including:
    - Total Events Count
    ...
    **Default Behavior**: If no date parameters are provided, returns data for the last 1 year.
    ...
    """
    try:
        logger.info(f"SRS Enriched Dashboard request - Start: {start_date}, End: {end_date}, Role: {user_role}, Region: {region}")
        
        # Initialize dashboard service
        dashboard_service = SRSEnrichedDashboardService()
        ...
        logger.info(f"Successfully generated SRS enriched dashboard data for {dashboard_data['date_range']}")
        return dashboard_data
        
    except ValueError as ve:
        logger.error(f"Validation error in SRS enriched dashboard: {ve}")
        raise HTTPException(status_code=400, detail=str(ve))
    except Exception as e:
        logger.error(f"Error in SRS enriched dashboard endpoint: {e}")
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to retrieve SRS enriched dashboard data: {str(e)}"
        )


@router.get("/srs-enriched/summary")
async def get_srs_enriched_dashboard_summary(
    ...
    """
    Get SRS enriched dashboard summary with key metrics only
    ...
    try:
        ...
        logger.error(f"Error in SRS enriched dashboard summary endpoint: {e}")
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to retrieve SRS enriched dashboard summary: {str(e)}"
        ) 