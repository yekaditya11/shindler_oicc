/**
 * NI TCT Dashboard Charts Component
 * Displays standardized dashboard data for NI TCT (Non-Intrusive Testing) events
 * 12 KPIs with consistent styling patterns + special time pattern analysis
 */
...
// Color scheme consistent with other dashboards - NI TCT themed
const colors = {
  primary: '#0284c7', // Blue theme for NI TCT
  secondary: '#059669',
  success: '#059669',
  warning: '#d97706',
  error: '#dc2626',
...
const NITCTDashboardCharts = ({ data }) => {
  ...
  // Monthly trends chart - Multi-line chart (matching EI Tech)
      ...
      itemStyle: {
        color: {
          ...
          colorStops: [
            ...
            { offset: 1, color: alpha(colors.primary, 0.6) }
          ]
        },
  ...
  // Time-based analysis chart (unique to NI TCT)
  const timeBasedChart = {
    tooltip: {
      trigger: 'axis',
      formatter: 'Time {b}: {c} events'
    },
    xAxis: {
      type: 'category',
      data: dashboardData.time_based_analysis?.time_of_day_analysis?.map(item => item.time_period) || [],
      axisLabel: { color: '#666' }
    },
    yAxis: {
      type: 'value',
    ...
    series: [{
      data: dashboardData.time_based_analysis?.time_of_day_analysis?.map(item => item.incident_count) || [],
      type: 'bar',
      itemStyle: {
        color: {
          type: 'linear',
          ...
          colorStops: [
            ...
            { offset: 1, color: alpha(colors.secondary, 0.6) }
          ]
        },
        borderRadius: [4, 4, 0, 0]
      }
  ...
  // Branch Analysis - Multi-Bar Chart (All Branches)
  const branchAnalysisChart = {
    tooltip: {
      trigger: 'axis',
      axisPointer: { type: 'shadow' }
    },
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <ChartCard title="Branch Analysis" height={400} icon={<BusinessIcon />}>
              <ReactECharts option={branchAnalysisChart} style={{ height: '100%' }} />
            </ChartCard>
          </Grid>
        </Grid>
      </Box>
...
export default NITCTDashboardCharts; 